...
void setup() {
  ...
  // Making new buttons
  btn1 = new Button(100, height/2, 100, 50, "Option 1", 120, 139, 201); //Position, Width & Height, Text and colors
...
void draw() {
  ...
  text(v1+v2+v3+v4+v5 + " vote(s) so far ", width/2, 100); //Feedback for the user when pressing the buttons
 ...
 if (v1 + v2 + v3 + v4 + v5 >= 10) { //Switching to the bar chart screen
   screen = 1;
 }
 }
 
 if (screen == 1) { // Drawing the the bar graph when on screen1 with the help of user defined funtion
   background(120, 139, 201);
   drawCoordinate();
   drawBars();
 }
